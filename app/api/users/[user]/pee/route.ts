...
/**
 * run9's write path: log one occurrence, or take the last one back.
 *
 * A SECOND ROUTE ALONGSIDE THE WALK ONE, not a refactor of it into something
 * shared. docs/dashboard-build-rules.md §4 is explicit that
 * app/api/users/[user]/walk/route.ts is the worked example rather than a base
 * class: the four ordered checks below are the security property and are
 * cheaper to read twice than to trace through an abstraction.
 *
 * The order of the checks IS that property:
 *
...
export async function POST(
  ...
  const panel = action === 'add' ? 'pee_log' : 'pee_correction'
  ...
  // Host-relative: the app runs behind a reverse proxy, so request.url names
  // the internal origin. See lib/http/redirect.ts.

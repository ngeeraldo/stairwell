...
/**
 * run11's spending screen: re-file a transaction, or create a bucket.
 *
 ...
 *
 * ─── TWO ACTIONS, ONE ROUTE, WHICH IS THE OPPOSITE CALL FROM run11's OTHER
 *     TWO ROUTES ────────────────────────────────────────────────────────────
 *
 * app/api/users/[user]/walk-log/route.ts and .../no-go-temp/route.ts are
 * deliberately separate because lib/ui/WriteAction.tsx groups its pending state
 * by ACTION URL, and the calendar squares and the temperature stepper have no
 * business locking each other.
 *
 * Here the grouping is the reason to MERGE rather than to split. Both actions
 * change the same surface: re-filing moves a slice, and creating a bucket adds
 * an option to every select in the list. Every control on the Spending screen
 * should therefore go pending together and settle together, because the pie
 * and the list are drawn from one read and must never be seen disagreeing.
 * Splitting these into two routes would let a friend create a category while a
 * re-file was still in flight and watch half the screen update.
 *
 * ─── THIS ROUTE WRITES FREE TEXT, WHICH NO ROUTE IN THIS REPO HAS DONE ─────
 *
 * `name` on the create path is a string the friend TYPES. Every other write
 * route in this app takes a closed set (a direction, a verb) or a day key.
 * What that changes, and what it does not:
 *
 *   * It is bounded and normalised before it is stored — trimmed, collapsed of
 *     inner whitespace, rejected if empty or longer than MAX_NAME. An
 *     unbounded string in a TEXT PRIMARY KEY is a row nothing can render.
 ...
 *     the transaction id. CLAUDE.md's own example of what may not be written
 *     to that unencrypted table is a bucket called `divorce_lawyer_fund`, and
 *     this is the column that would hold one.
 *   * It is never logged. `logDbFailure` is handed the error, never the body.
 *
 * ─── AND IT TAKES A transaction_id FROM THE CALLER ─────────────────────────
 *
 * Unavoidable: re-filing names a specific transaction. Unlike the walk log's
 * `day` this is NOT a primary key in a table the friend can corrupt by sending
 * a malformed one — an override row keyed to an id no transaction has is inert,
 * because every read joins FROM the transaction (004's `spending_transactions`).
 * It is still bounded, and it is checked against the friend's OWN synced rows
 * before anything is written, so this route cannot be used to accumulate rows
 * for ids that were never his.
 *
 ...
 *
 * RESTATED HERE rather than imported from users/run11/queries.ts, exactly as
 * app/api/users/[user]/no-go-temp/route.ts restates its own four numbers: a
 * platform route importing a user folder would make one friend's dashboard a
 * build dependency of the platform. `users/run11/tests/spendingCategory.test.ts`
 ...
 * "Eating out" are the same bucket to the friend and two rows to SQLite, and
 * he would have no way to tell them apart on screen. Control characters go the
 * same way — a name carrying a newline renders as a broken row in a select.
 */
...
export async function POST(
  ...
  //
  // `show` and `hide` are two actions rather than one `toggle`, deliberately:
  // a toggle computes the new state from whatever it finds, so two presses
  // racing each other can land in the state neither of them asked for. Naming
  // the target state makes the request idempotent — the same reason
  // app/api/users/[user]/no-go-temp/route.ts takes a direction rather than a
  // number, arrived at from the opposite end.
  ...
  // A File, or a missing field, is not a string — checked first so nothing
  // below is ever handed a non-string.
  let transactionId = ''
  let category = ''
  let name = ''
  if (action === 'assign') {
    ...
    // does. It is chosen from a menu rather than typed, but the no-JS path
    // posts whatever the form holds, and a category assigned as "Eating  out"
    // would be a slice that never joins the bucket it came from.
  ...
  try {
    if (action === 'assign') {
      // THE TRANSACTION HAS TO BE HIS, and in scope. `spending_transactions`
      // is 004's view, which already restricts to the accounts this screen
      // covers — so this one check answers both "does it exist" and "is it a
      // transaction the Spending screen is allowed to re-file". Without it,
      // this route would accumulate override rows for arbitrary ids: inert,
      // but unbounded growth in a database only the friend can ever clean up.
      const known = userDb
      ...
      // THE CATEGORY HAS TO BE ONE HE ACTUALLY HAS. Same job as the
      // transaction check above, and the same reason: without it this route
      // accumulates rows for categories that do not exist, in a database only
      // the friend can ever clean up. A category is real if some transaction
      // currently sits in it, or if it is a bucket he made — the second arm
      // matters because a brand-new bucket has nothing in it yet.
      const known = userDb
        .prepare(
      ...
      // the request named, never a flip of what was found — see the action
      // check above.
      userDb
        .prepare(
      ...
      // OR IGNORE, and the reasoning is 002's walk_log again: typing a name
      // that already exists is the same intent as having created it, so it is
      // a no-op rather than an error the friend has to understand. The column
      // is COLLATE NOCASE (004), so "Coffee" does not become a second
      // "coffee".
      userDb
  ...
  // Permanent policy: a slug and a panel, never a value. NOT the category
  // name, NOT the transaction id. This is the first route in the repo whose
  // payload is free text the friend composed, so it is the row where that
  // policy would be easiest to break and hardest to notice — `metrics` is the
  // unencrypted platform database, and this row is what makes the login page's
  // "I can see when you use it ... but not what you log" true. Two panel names
  // rather than one reused: re-filing and creating a bucket are distinct
  // events, and a query grouping by panel needs to tell them apart.
  appendMetric(db, {
    accountId,
  ...
  // dashboard_open row.
  //
  // The redirect names the spending screen, not the bare dashboard: without
  // JavaScript this is the whole navigation, and landing a friend who just
  // re-filed a transaction back on the decider would look like the change was
  // thrown away.

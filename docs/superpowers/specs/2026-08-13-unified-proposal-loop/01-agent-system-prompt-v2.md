# Agent system prompt — v2

> Handoff note (not part of the prompt): this replaces the current prompt wholesale. The
> structural changes it assumes — one versioned whole-surface spec per user, no
> tweak/build distinction — are described in `02-system-structure-changes.md`. The spec
> shape it references is defined in `03-spec-schema.md`.

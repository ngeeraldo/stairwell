...
// users/run11/CategoryControls.tsx
//
// The three controls on the Spending screen that lib/ui/WriteAction.tsx cannot
// express: a per-transaction category MENU, a text field for naming a new
// bucket, and the legend's TICK BOX for keeping a category out of the pie. All
// write through the same platform route, and all are built on the platform's
// own mechanics rather than on a second copy of them.
//
// ─── why not WriteAction ───────────────────────────────────────────────────
//
// WriteAction is the default for every write in this repo and is used
// unchanged everywhere else on this dashboard — the Refresh control on this
// very screen is one. But it renders a labelled BUTTON over a fixed payload,
// and this screen asks for three things a button cannot carry: a choice among
// categories, a name the friend types, and a tick box that shows its own
// state.
//
// lib/ui/useWriteAction.ts is the sanctioned answer, and says so in its own
// header: "Exported as an escape hatch for anything a labelled button cannot
// express — a form with fields, say. No such case exists today; WriteAction is
// the expected entry point, and this exists so that the first dashboard needing
// more does not reimplement the lifetime rule below." This is that first case.
//
...
//
// ─── what the grouping buys, and why both controls share one route ─────────
//
// `useWriteAction` locks every control sharing an ACTION URL while a write is
// in flight. Both controls here post to the same route, so re-filing a
// transaction locks every other row's menu AND the new-bucket field until the
// server has answered and the tree has re-rendered.
//
// That is correct rather than incidental: the pie above is drawn from the same
// read as the list, so a second write landing while the first was settling
// would show a friend a pie and a list that disagreed. It is the same call
// ./MonthCalendar.tsx's forty squares make, and the opposite of the one
// app/api/users/[user]/no-go-temp/route.ts makes by staying a separate route —
// there, two controls with nothing to do with each other should not lock.
//
// ─── the no-JS path is real ────────────────────────────────────────────────
//
// Both render a genuine <form method="post" action={...}> with real named
...
// enhancement, not the mechanism. That is why the submit button below is a real
// submit and not an onClick.
//
// ─── this file writes no free text anywhere but the friend's own database ──
//
// The name typed below reaches exactly one place: `custom_categories` in his
// own SQLCipher database, through the route. It is not logged, and the route
// emits a constant panel name to `metrics` rather than anything derived from
...
export function RefileControl({
  ...
  // say. Kept as an option rather than silently snapping the select to
  // something else, which would show the friend a category the row is not in.
 ...
 *
 * Spec v3 does not ask for this — it is Nico's call during the build review,
 * and it exists to answer that spec's own second open question about transfers
 * and card payments dominating a spending breakdown. Hard-coding which
 * categories "are not really spending" would be this dashboard forming an
 * opinion about the friend's money; this hands him the decision.
 *

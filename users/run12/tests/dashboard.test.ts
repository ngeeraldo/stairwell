...
// about run12's dashboard, because that has not been designed yet. Replace
...
describe('users/run12', () => {
  it('says it is not built yet', () => {
    const db = emptyDbFromMigrations('run12')
    try {
      const json = JSON.stringify(
        Dashboard({ slug: 'run12', db, today: '2026-01-01', now: Date.parse('2026-01-01T12:00:00Z'), timeZone: 'UTC' }),
      )
      expect(json).toContain('Under construction')
      expect(json).toContain('run12')
    } finally {
      db.close()
    }
  })
...
//   // import { someQuery, annotatedRows } from '@/users/run12/queries'
//
//   let writeDb: UserDb
//
//   beforeEach(() => {
//     writeDb = new Database(':memory:')
//     applyUserMigrations(writeDb, 'run12')
//   })
//   afterEach(() => {
//     writeDb.close()
//   })

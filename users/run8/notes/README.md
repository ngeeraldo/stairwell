...
**Never put run8's data in here.** This folder is committed to the repo.
...
Two sections reach run8 and two do not. `lib/build/notes.ts` enforces that
...
## What shipped
Product-level. What run8 can now see or do that they could not before.
...
instruction rather than a disclosure: it never reaches run8. Take it back

...
import {
  LEGACY_SPEC_JSON_SCHEMA,
  parseLegacySpecInput,
  parseLegacySpecPayload,
} from '@/lib/spec/legacy'
...
describe('parseLegacySpecInput', () => {
  it('accepts a well-formed payload and splits the mockup out', () => {
    const { payload, mockupHtml } = parseLegacySpecInput(good())
  ...
  it('trims whitespace and drops blank list entries', () => {
    const { payload } = parseLegacySpecInput(
  ...
  it.each([
    ...
    expect(() => parseLegacySpecInput(raw)).toThrow(SpecShapeError)
  })
...
describe('parseLegacySpecPayload', () => {
  it('accepts a well-formed stored payload JSON', () => {
    ...
    const result = parseLegacySpecPayload(json)
  ...
  it('rejects a stored payload that is malformed in a payload field', () => {
    ...
    expect(() => parseLegacySpecPayload(json)).toThrow(SpecShapeError)
  })

  it('throws SpecShapeError specifically when JSON is malformed', () => {
    const invalidJson = '{"title": "broken'
    expect(() => parseLegacySpecPayload(invalidJson)).toThrow(SpecShapeError)
  })
})

describe('LEGACY_SPEC_JSON_SCHEMA', () => {
  it('requires exactly the fields the validator requires', () => {
    ...
    expect([...LEGACY_SPEC_JSON_SCHEMA.required].sort()).toEqual([
      'background',
      'manual_logging',
      'mockup_html',
      'open_questions',
    ...
    expect(Object.keys(LEGACY_SPEC_JSON_SCHEMA.properties).sort()).toEqual(
      [...LEGACY_SPEC_JSON_SCHEMA.required].sort(),
    )
  })

  it('pins the panel source enum to the three real sources', () => {
    expect(
      LEGACY_SPEC_JSON_SCHEMA.properties.panels.items.properties.source.enum,

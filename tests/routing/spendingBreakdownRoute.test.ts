// tests/routing/spendingBreakdownRoute.test.ts
//
// run12's spending write path — the ONLY thing pinning
// app/api/users/[user]/spending-breakdown/route.ts. users/run12/tests/queries.test.ts
...
//
// ─── WHY THIS ROUTE EXISTS AT ALL, GIVEN run11 HAS A NEAR-TWIN ────────────
//
// app/api/users/[user]/spending-category/route.ts is run11's, is written
// against a `[user]` path, and would serve run12 verbatim. It is deliberately
// not reused — see that route's own header, and
// app/api/users/[user]/pee-log/route.ts, which makes the same call for the same
// reason: "a shared handler would make a change to one friend's dashboard a
// silent change to another's". This file is therefore a near-twin of
// tests/routing/spendingCategoryRoute.test.ts on purpose, and the duplication is
...
/**
 * run12's REAL migrations, read off disk rather than retyped, so this suite
 * exercises the shape run12.db is actually created under. ALL FOUR: the Plaid
 * envelope arrives in 001-002, the views in 003, and the three tables this
 * route writes in 004, so a fixture
 * stopping at 002 would prove nothing about the tables this route touches.
 */
const MIGRATIONS_SRC = resolve(__dirname, '..', '..', 'users', 'run12', 'migrations')
const MIGRATIONS = [
...
/** An in-scope transfer, which 004 flags internal until he re-files it. */
const TRANSFER = 'txn-transfer-TEST'

beforeEach(() => {
  dir = mkdtempSync(join(tmpdir(), 'stairwell-run12-spending-'))
  ...
  // against run12's real data on the droplet. lib/db/userData.ts sends dev to
  ...
  mkdirSync(join(dir, 'users', 'run12', 'migrations'), { recursive: true })
  for (const m of MIGRATIONS) {
    writeFileSync(join(dir, 'users', 'run12', 'migrations', m.name), m.sql)
  }
  writeFileSync(
    join(dir, 'users', 'run12', 'migrations', 'manifest.json'),
    JSON.stringify({
...
function openUserDb(slug = 'run12') {
 ...
 * way 003's `spending_accounts` view cares about.
 *
...
function seedTransactions() {
  ...
  try {
    ...
    // A transfer on the CARD, so it is in scope for the screen and flagged
    // internal by 004. It is what the override-clears-the-flag test moves.
    db.prepare(
      'INSERT INTO plaid_transactions (transaction_id, account_id, date, payload) VALUES (?, ?, ?, ?)',
    ).run(
      TRANSFER,
...
/**
 * Sign run12 in. `lock` withholds the key, as a restart would; `migrate: false`
...
async function arrange(
  ...
  if (!opts.lock) {
    ...
    if (opts.migrate !== false) {
      ...
      migrateUserDb(opts.slug ?? 'run12', KEY)
      if (opts.seed !== false) seedTransactions()
    }
  }
  cookieSlot.value = { value: sid }
  const { POST } = await import('@/app/api/users/[user]/spending-breakdown/route')
  return POST
}
...
describe('POST /api/users/[user]/spending-breakdown — the four ordered checks', () => {
  it('refuses a LOCKED session before it touches ownership or a file', async () => {
    const POST = await arrange({ lock: true })
    const res = await POST(submit({ action: 'create', name: 'Coffee' }), params('run12'))
    ...
    expect(existsSync(join(dir, 'users', 'run12', 'run12.db'))).toBe(false)
  })
...
describe('POST /api/users/[user]/spending-breakdown — what it will not accept', () => {
  it('refuses an unknown action', async () => {
    const POST = await arrange()
    const res = await POST(submit({ action: 'delete', name: 'Coffee' }), params('run12'))
  ...
  it('refuses an empty or whitespace-only category name', async () => {
    const POST = await arrange()
    for (const name of ['', '   ', '\n\t ']) {
      const res = await POST(submit({ action: 'create', name }), params('run12'))
      expect(res.status).toBe(400)
    }
    expect(categories()).toEqual([])
  })
  ...
  it('refuses a name longer than the bound, measured AFTER normalisation', async () => {
    ...
    // nothing can render.
    const res = await POST(submit({ action: 'create', name: 'x'.repeat(41) }), params('run12'))
    expect(res.status).toBe(400)
...
describe('POST /api/users/[user]/spending-breakdown — re-filing', () => {
...
describe('POST /api/users/[user]/spending-breakdown — creating a bucket', () => {
  ...
  it('is a no-op on a name that already exists, in any case', async () => {
    ...
    const POST = await arrange()
    await POST(fetchSubmit({ action: 'create', name: 'Coffee' }), params('run12'))
    const res = await POST(fetchSubmit({ action: 'create', name: 'coffee' }), params('run12'))
    expect(res.status).toBe(204)
...
describe('POST /api/users/[user]/spending-breakdown — ticking a category', () => {
  ...
  it('stores a tick for a bucket that has nothing in it yet', async () => {
    ...
    const POST = await arrange()
    await POST(fetchSubmit({ action: 'create', name: 'Eating out' }), params('run12'))
    const res = await POST(
  ...
  it('upserts, and the LAST request wins', async () => {
    const POST = await arrange()
    await POST(fetchSubmit({ action: 'hide', category: 'FOOD_AND_DRINK' }), params('run12'))
    await POST(fetchSubmit({ action: 'show', category: 'FOOD_AND_DRINK' }), params('run12'))
  ...
  it('snaps a bucket name to the casing he stored it under', async () => {
    ...
    const POST = await arrange()
    await POST(fetchSubmit({ action: 'create', name: 'Eating out' }), params('run12'))
    await POST(fetchSubmit({ action: 'hide', category: 'eating OUT' }), params('run12'))
...
describe('POST /api/users/[user]/spending-breakdown — the metric row', () => {
    ...
    expect(rows).toEqual([
      {
        ...
        data: { slug: 'run12', panel: 'spending_category_create', device_class: 'desktop' },
      },
      {
        event: 'dashboard_write',
        data: { slug: 'run12', panel: 'spending_category_assign', device_class: 'desktop' },
      },
    ])
  ...
  it('names the two actions apart, so a query can tell them from each other', async () => {
    ...
    await POST(fetchSubmit({ action: 'hide', category: 'Coffee' }), params('run12'))
    await POST(fetchSubmit({ action: 'show', category: 'Coffee' }), params('run12'))
  ...
  it('writes NO metric row when the request is refused', async () => {
    const POST = await arrange()
    await POST(submit({ action: 'create', name: '  ' }), params('run12'))
    expect(metricRows()).toEqual([])
  })
})

describe('POST /api/users/[user]/spending-breakdown — how it answers', () => {
  it('redirects a native form post back to the spending screen', async () => {
    ...
    expect(res.headers.get('location')).toBe('/run12?screen=spending')
  })
...
// ─────────────────────────────────────────────────────────────────────────────
// THE TWO THINGS THAT ARE run12'S OWN, NOT INHERITED FROM run11's SHAPE
// ─────────────────────────────────────────────────────────────────────────────

describe('POST /api/users/[user]/spending-breakdown — run12 specifics', () => {
  it('keeps its restated MAX_NAME in step with users/run12/queries.ts', () => {
    // The route may not IMPORT the user folder — a platform route importing one
    // friend's dashboard would make it a build dependency of the platform — so
    // the bound is written twice on purpose. This is the thing that keeps the
    // duplication honest, and the route's own header promises it exists.
    const source = readFileSync(
      resolve(__dirname, '..', '..', 'app', 'api', 'users', '[user]', 'spending-breakdown', 'route.ts'),
      'utf8',
    )
    const declared = /^const MAX_NAME = (\d+)$/m.exec(source)
    expect(declared, 'route.ts must declare MAX_NAME on its own line').not.toBeNull()

    const queries = readFileSync(
      resolve(__dirname, '..', '..', 'users', 'run12', 'queries.ts'),
      'utf8',
    )
    const exported = /^export const CATEGORY_NAME_MAX = (\d+)$/m.exec(queries)
    expect(exported, 'queries.ts must export CATEGORY_NAME_MAX').not.toBeNull()

    expect(Number(declared![1])).toBe(Number(exported![1]))
  })

  it('makes a re-filed transfer COUNT, because an override clears is_internal', () => {
    // 004's most arguable line, proved end to end through the route rather than
    // through a hand-written override row. A control that silently did nothing
    // for one class of row would be a broken control: he moves a transfer,
    // presses Move, and the pie does not change with nothing explaining why.
    //
    // Deliberately asserted on the VIEW, not on the override table: the table
    // says he pressed the button, the view says the screen changed.
    return (async () => {
      const POST = await arrange()
      const before = openUserDb()
      try {
        expect(
          before
            .prepare('SELECT is_internal FROM spending_transactions WHERE transaction_id = ?')
            .get(TRANSFER),
        ).toEqual({ is_internal: 1 })
      } finally {
        before.close()
      }

      const res = await POST(
        fetchSubmit({ action: 'assign', transaction_id: TRANSFER, category: 'GUILT FREE TEST' }),
        params('run12'),
      )
      expect(res.status).toBe(204)

      const after = openUserDb()
      try {
        expect(
          after
            .prepare(
              'SELECT category, is_internal FROM spending_transactions WHERE transaction_id = ?',
            )
            .get(TRANSFER),
        ).toEqual({ category: 'GUILT FREE TEST', is_internal: 0 })
      } finally {
        after.close()
      }
    })()
  ...
  it('never lets a typed name reach the metrics table', () => {
    // `metrics` is the UNENCRYPTED platform database. This route is the only one
    // on run12's dashboard whose payload is free text the friend composed, so it
    // is where that policy would be easiest to break and hardest to notice.
    // CLAUDE.md's own example of what may not be written there is a bucket
    // called `divorce_lawyer_fund`; this is the column that would hold one.
    return (async () => {
      const POST = await arrange()
      const secret = 'DIVORCE LAWYER FUND TEST'
      await POST(fetchSubmit({ action: 'create', name: secret }), params('run12'))
      await POST(
        fetchSubmit({ action: 'assign', transaction_id: IN_SCOPE, category: secret }),
        params('run12'),
      )
      await POST(fetchSubmit({ action: 'hide', category: secret }), params('run12'))

      // It really was stored — otherwise this test would pass on a route that
      // did nothing at all.
      expect(categories()).toContain(secret)

      const rows = metricRows()
      expect(rows.map((r) => r.data.panel)).toEqual([
        'spending_category_create',
        'spending_category_assign',
        'spending_category_hide',
      ])
      // Whole-row assertion, and then the blunt one: the name appears nowhere
      // in the serialised metrics table, under any key.
      for (const row of rows) {
        expect(row.data).toEqual({ slug: 'run12', panel: row.data.panel, device_class: 'desktop' })
      }
      const raw = handle!.prepare('SELECT data FROM metrics').all() as { data: string }[]
      expect(raw.map((r) => r.data).join(' ')).not.toContain(secret)
      expect(raw.map((r) => r.data).join(' ')).not.toContain(IN_SCOPE)
    })()
  })
})

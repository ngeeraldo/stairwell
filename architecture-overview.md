...
- **`propose_spec` asks the writer for only the change, and the stored row is
  only the change too.** The base it writes against is
  `users/<slug>/current.md` — the dashboard as it was actually built — not the
  previous spec row, and the shape it authors is `lib/spec/change.ts`'s:
  `change_summary`, `changes[]`, `data_requirements`, `open_questions`, with no
  ids and no restated surface. There is ONE authoring path; the patch/whole
  split, the eight ops and `applyPatch` are gone, and so is the whole-surface
  authoring contract (`SPEC_JSON_SCHEMA`, `parseSpecDraft`). Their readers are
  frozen rather than deleted, because `specs` rejects UPDATE and rows in the
  older shapes must keep parsing forever — which is also why
  `lib/spec/stored.ts` now has three arms (`change`, `version`, `legacy`),
  discriminating on a server-written `payload.shape` tag checked before the
  `screens` array.
- **There is no mockup any more, and nothing is scoped for a preview's
  ...
  nothing renders model-generated HTML to a person any more. The change-only
  shape above still bounds what the WRITER touches; it no longer needs to also
...
- **What each version CHANGED, first-class.** Screens and panels added, removed or changed is the canonical record of what a request was. It replaces classifying chat text after the fact, and it is what settles the "expressible as config" vs. "needed custom code" question — the distribution that decides the future architecture debate. Since change-only specs the spec row IS that record — its `changes[]` is the diff, so nothing computes one (`lib/spec/diff.ts` is frozen, and still diffs the whole-surface rows the admin pane renders for accounts that have them).
- **`authoring_mode` and `changes_count`, first-class alongside it.** Every metric row `lib/spec/author.ts` writes carries `authoring_mode` — now the constant `'change'`, kept so `patch`, `whole` and `change` read as three eras of one field — and `changes_count`, how many changes the version proposed (`null` when the draft never parsed). `ops_count` is gone with the ops. This is what makes the token-cost claim below checkable after the fact rather than asserted: a run's cost can be read off against the size of the change it proposed.
